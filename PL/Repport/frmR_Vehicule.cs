...
{
    public partial class frmR_Vehicule : Form
    {
        #region Variables
        ...
        {
            var rs = db.Select_vwArticle_Info(idService);
        ...
        private void search()
        {
            try
            {
                dgvStock.DataSource = db.Search_R_Article(txtSearch.Text, cmbxTable.Text, cmbxCondition.Text, idService).ToList();
            }
            catch (Exception)
            {
            }
