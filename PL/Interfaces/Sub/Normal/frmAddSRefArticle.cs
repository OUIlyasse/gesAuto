...
{
    public partial class frmAddSRefArticle : Form
    {
        #region Fields
        ...
        private decimal qte, qte_Ref_EnStock;
        ...
        private void LoadReference_Article()
        {
            try
            {
                var data = from t in db.Bon_reference where t.art_ID == idArticle && t.br_Type == "S" && t.bon_ID == idBon && t.bs_A_ID == idBonArt select new { t.br_ID, t.br_Type, t.art_ID, t.Article_Reference.ref_reference, t.bon_ID, t.br_Qte, t.bs_A_ID, t.br_Status_Gravage };
        ...
        private string getReferenceByID(int ref_ID)
        {
            string AssNo = (from a in db.Article_Reference
                            where a.ref_ID == ref_ID
                            select a).Single().ref_reference;
            return AssNo;
        }

        private string getVLByID(int vl_ID)
        {
            string AssNo = (from a in db.Vehicules
                            where a.vl_ID == vl_ID
                            select a).Single().vl_Marque;
            return AssNo;
        }
        ...
        {
            var _result = from x in db.Bon_Sortie_Article
                          where x.vl_ID == idVL
                          where x.art_ID == idArticle
                          where x.bs_ID == idBon
                          select x;
        ...
        private void Add_data()
        {
            using (var transaction = db.Database.BeginTransaction())
            {
                try
                {
                    ...
                    decimal qteR = Convert.ToDecimal(txtQteDemandee.Text);
                    if (qte_Ref_EnStock < qteR)
                    {
                    ...
                    int rs = (int)db.Insert_Bon_reference(max, "S", idArticle, (int)cmbxReference.SelectedValue, idBon, decimal.Parse(txtQteDemandee.Text), idBonArt, null, null, status).FirstOrDefault();
                    switch (rs)
                    {
                        case 1:
                            transaction.Commit();
        ...
        private void Update_data()
        {
            using (var transaction = db.Database.BeginTransaction())
            {
                try
                {
                    ...
                    int max = bonRef;
                    int rs = (int)db.Update_Bon_reference(max, "S", idArticle, (int)cmbxReference.SelectedValue, idBon, decimal.Parse(txtQteDemandee.Text), idBonArt, null, null, status).FirstOrDefault();
                    switch (rs)
                    {
                        case 1:
                            transaction.Commit();
        ...
        public frmAddSRefArticle(int idArt, int idBon, int idVL, frmAddSArticle grid, decimal qte, int idBonArt)
        {
            InitializeComponent();
            idArticle = idArt;
            this.idBon = idBon;
        ...
        private void cmbxReference_Validating(object sender, CancelEventArgs e)
        {
            if (cmbxReference.SelectedIndex != -1)
            {
                var qte = db.getQuantite(getVLByID(idVL), getArticleByID(idArticle), getReferenceByID(int.Parse(cmbxReference.SelectedValue.ToString()))).FirstOrDefault();
                if (qte == null)
                {
                    qte_Ref_EnStock = 0;
                ...
                {
                    qte_Ref_EnStock = (decimal)qte;
                    lblQteTotal.Text = $"En stock: {qte}";
                }
            }
        }
        ...
        public frmAddSRefArticle(int idArt, int idBon, int idVL, frmAddSArticle grid, decimal qte, int idBonArt, int refID, string qteRef, int bonRef, bool StatusGravage)
        {
            InitializeComponent();
            idArticle = idArt;
            this.idBon = idBon;
