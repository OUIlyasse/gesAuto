...
{
    public partial class frmCodeBarreRAvancee : Form
    {
        #region Fields
        ...
        public frmCodeBarreRAvancee(string Article, string Abreviation, string QteArticle, string Reference, string QteReference, Bon_Retour_Article bsa, Bon_reference bre, frmAddRArticle f)
        {
            InitializeComponent();
            LoadAnnee();
            ...
            using (var transaction = db.Database.BeginTransaction())
            {
                try
                {
                    ...
                    foreach (var x in list)
                    {
                        ...
                        AddOrUpdatePosition(x.Value, "S", (int)dd.pa_Numero, (int)dd.pa_Annee, (int)dd.ref_ID, (int)dd.art_ID);
                    }
        ...
        private void UpdateBon_Reference()
        {
            try
            {
                ...
                int rs = (int)db.Update_Bon_reference(idBonRef, "R", idart, idReff, idbon, qtee, bsa, null, null, state).FirstOrDefault();
                switch (rs)
                {
                    case 1:
                        //form.txtStatus.Caption = "Votre code a bien ajouté";
        ...
        private void LoadCodBarre_Reference()
        {
            try
            {
                ...
                var data = from t in db.Bon_CodeBarre where t.bc_Date == bonArt.Bon_Retour.bt_Date && t.art_ID == bonArt.art_ID && t.bc_Type == "R" && t.bon_ID == bonArt.bt_ID && t.vl_ID == id select new { t.bc_ID, t.bc_N_Serie, t.ref_ID };
        ...
        private void LoadReference_Article()
        {
            try
            {
                var data = from t in db.Bon_reference where t.art_ID == bonArt.art_ID && t.br_Type == "R" && t.bon_ID == bonArt.bt_ID && t.bs_A_ID == bonArt.bt_A_ID select new { t.br_ID, t.br_Type, t.art_ID, t.Article_Reference.ref_reference, t.bon_ID, t.br_Qte, t.bt_A_ID, t.Article_Reference.ref_est_Gravee };
        ...
        private void CodeBarreGenerateur(int refID, int vlID, int idArticle, int idBon)
        {
            try
            {
                ...
                for (int i = 0; i < qte; i++)
                {
                    ...
                    int rs = (int)db.Insert_Bon_CodeBarre(id, DateTime.Now, "R", format, x, int.Parse(cmbxAnnee.Text), refID, idArticle, idBon, vlID).FirstOrDefault();
                    switch (rs)
                    {
                        case 1:
                            Bon_CodeBarre refArt = db.Get_Bon_CodeBarre_By_ID(id).FirstOrDefault();
                            AddOrUpdatePosition(refArt.bc_N_Serie, "R", (int)refArt.bc_Numero, (int)refArt.bc_Annee, (int)refArt.ref_ID, (int)refArt.art_ID);
                            break;
                    }
                }
                UpdateBon_Reference();
